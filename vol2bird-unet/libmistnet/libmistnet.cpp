...
extern "C" int run_mistnet(float* tensor_in, float** tensor_out, const char* model_path, int tensor_size) {
        ...
        // copy result. The U-Net segmentation model outputs 2 classes
        // (non-biological, biological) per elevation, class-major:
        // [class][elev][x][y] = 2 x 5 x 608 x 608.
        for (int i=0; i<2*5*608*608; i++) (*tensor_out)[i] = output_array[i];
