    ...
    for (int iScan = 0; iScan < nScans; iScan++) {
        ...
        for(int iRang=0; iRang<nRang; iRang++){
            for(int iAzim=0; iAzim<nAzim; iAzim++){
                ...
                float valueNonBiology=tensor[MISTNET_NONBIOLOGY_INDEX][iScan][x][y];
                float valueBiologyAvg=0;
                for(int i=0; i<nScans; i++){
                    valueBiologyAvg+=(tensor[MISTNET_BIOLOGY_INDEX][i][x][y]/nScans);
                }
                int valueClassification = CELLINIT;
                // post-processing rule: a pixel is non-biological (weather/clutter)
                // when its biological probability is low, either at this elevation
                // or on average across all elevations at this location.
                // Adapted from Lin et al. 2019 (doi 10.1111/2041-210X.13280) for a
                // 2-class biological / non-biological U-Net segmentation.
                if(valueBiology < MISTNET_BIOLOGY_THRESHOLD || valueBiologyAvg < MISTNET_SCAN_AVERAGE_BIOLOGY_THRESHOLD){
                    valueClassification=MISTNET_WEATHER_CELL_VALUE;
                }
                PolarScanParam_setValue(mistnetParamBiology, iRang, iAzim, valueBiology);
                PolarScanParam_setValue(mistnetParamWeather, iRang, iAzim, valueNonBiology);
        ...
        for(int iRang=0; iRang<nRang; iRang++){
            for(int iAzim=0; iAzim<nAzim; iAzim++){
                ...
                for(int i=0; i<dim2; i++){
                    valueBiologyAvg+=(tensor[MISTNET_BIOLOGY_INDEX][i][x][y]/dim2);
                }
                int valueClassification = CELLINIT;
                // post-processing rule for scans not part of the segmentation model:
                // mark as non-biological where the cross-elevation average biological
                // probability is low. After Lin et al. 2019, doi 10.1111/2041-210X.13280,
                // adapted for the 2-class biological / non-biological U-Net.
                if(valueBiologyAvg < MISTNET_SCAN_AVERAGE_BIOLOGY_THRESHOLD){
                    valueClassification=MISTNET_WEATHER_CELL_VALUE;
                }
                PolarScanParam_setValue(mistnetParamClassification, iRang, iAzim, valueClassification);                
            }            
...
int segmentScansUsingMistnet(PolarVolume_t* volume, vol2birdScanUse_t *scanUse, vol2bird_t* alldata){    
    ...
    // the segmentation model outputs MISTNET_N_CLASS class-probability maps per
    // elevation (class-major: [class][elev][x][y]); size the output buffer to match.
    int mistnetOutputSize=MISTNET_N_CLASS*alldata->options.mistNetNElevs*MISTNET_DIMENSION*MISTNET_DIMENSION;
    float *mistnetTensorOutput = (float *) malloc(mistnetOutputSize*sizeof(float));
    ...
    float ****mistnetTensorOutput4D = create4DTensor(mistnetTensorOutput,MISTNET_N_CLASS,alldata->options.mistNetNElevs,MISTNET_DIMENSION,MISTNET_DIMENSION);
    // add segmentation to polar volume
    addTensorToPolarVolume(volume_mistnet, mistnetTensorOutput4D,MISTNET_N_CLASS,alldata->options.mistNetNElevs,MISTNET_DIMENSION,MISTNET_DIMENSION,MISTNET_RESOLUTION);
    ...
    addClassificationToPolarVolume(volume, mistnetTensorOutput4D,MISTNET_N_CLASS,alldata->options.mistNetNElevs,MISTNET_DIMENSION,MISTNET_DIMENSION,MISTNET_RESOLUTION);
    ...
    if(nCartesianParam > 0){
        ...
        free3DTensor(mistnetTensorInput3D,nCartesianParam,MISTNET_RESOLUTION);
        free4DTensor(mistnetTensorOutput4D, MISTNET_N_CLASS, alldata->options.mistNetNElevs, MISTNET_RESOLUTION);
    }
